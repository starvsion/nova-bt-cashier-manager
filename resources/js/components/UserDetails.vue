<script type="text/ecmascript-6">
  export default {
    ...
    methods: {
       ...
       */
      loadUserData () {
        let subscriptionId = _.isEmpty(this.newPlan) ? this.$route.query.subscription_id : this.newPlan;
        axios.get(`/nova-cashier-tool-api/billable/${this.userId}/?subscription_id=${subscriptionId}`)
          .then(response => {
            ...
            this.newPlan = response.data.subscriptions ? response.data.subscriptions.stripe_plan : null
    ...
    <div>
        ...
        <div class="card mb-6 py-3 px-6" v-if="!loading && subscription">
            <div class="flex border-b border-40">
                ...
                <div class="w-3/4 py-4"><p class="text-90">{{user.name}} <span v-if="user.email !== undefined">({{user.email}})</span>
                </p></div>
            </div>
                ...
                <div class="w-3/4 py-4">
                    <select v-model="newPlan" class="form-control form-select">
                        ...
                        <option :value="plan.id" v-for="plan in plans">
                            {{plan.description}} ({{plan.price}} {{plan.currency}} / {{plan.plan_interval}} time(s), on
                            the {{plan.plan_frequency}}th each month)
                        </option>
                    </select>

                    <button class="btn btn-sm btn-outline"
                            v-if="newPlan && newPlan != subscription.stripe_plan && subscription.active && !subscription.cancel_at_period_end"
            ...
            <div class="flex border-b border-40" v-if="subscription">
                ...
                <div class="w-3/4 py-4"><p class="text-90">{{subscription.plan_amount}} ({{subscription.plan_currency}})
                                                           / {{subscription.plan_interval}} time(s), on the
                                                           {{subscription.plan_frequency}}th each month</p></div>
            </div>
            ...
            <div class="flex border-b border-40 remove-bottom-border" v-if="subscription">
                ...
                <div class="w-3/4 py-4">
                    <p class="text-90">
                        ...
                        <button class="btn btn-sm btn-outline"
                                v-if="subscription.active && !subscription.cancelled && !subscription.cancel_at_period_end"
        ...
        <div class="card mb-6 relative" v-if="!loading && invoices && invoices.length">
            ...
            <div class="overflow-hidden overflow-x-auto relative">
                <table cellpadding="0" cellspacing="0" data-testid="resource-table" class="table w-full">
                    <thead>
                        <tr>
                            <th class="text-left"><span class="cursor-pointer inline-flex items-center">Amount</span>
                            </th>
                    ...
                    <tbody>
                        <tr v-for="invoice in invoices">
                            <td>
                                <span class="whitespace-no-wrap text-left">{{invoice.total / 100}} {{invoice.currency}}</span>
                            </td>
                            ...
                            <td class="text-right">
                                <button class="btn btn-sm btn-outline"
                                        v-on:click="refundCharge(invoice.charge_id)"
                ...
                <table cellpadding="0" cellspacing="0" data-testid="resource-table" class="table w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="text-left"><span class="cursor-pointer inline-flex items-center">Status</span>
                            </th>
                            <th class="text-left"><span class="cursor-pointer inline-flex items-center">Created</span>
                            </th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="charge in charges">
                            <td>
                                <span class="whitespace-no-wrap text-left">{{charge.amount / 100}} {{charge.currency}}</span>
                            </td>
                            <td>
        ...
        <div class="card mb-6 relative" v-if="!loading && cards && cards.length">
            ...
            <div class="overflow-hidden overflow-x-auto relative">
                <table cellpadding="0" cellspacing="0" data-testid="resource-table" class="table w-full">
                    <thead>
                        <tr>
                            <th class="text-left"><span class="cursor-pointer inline-flex items-center">Brand</span>
                            </th>
                            <th class="text-left"><span class="cursor-pointer inline-flex items-center">Number</span>
                            </th>
                            <th class="text-left">
                                <span class="cursor-pointer inline-flex items-center">Expiration</span></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="card in cards">
                            ...
                            <td><span class="whitespace-no-wrap text-left">{{card.exp_month}}/{{card.exp_year}}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
